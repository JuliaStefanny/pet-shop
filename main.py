...
def listar_produtos():
    ...
    if nome:
        lista_filtrada_por_nome = []
        for product in products:
            if product['nome'].lower() == nome.lower():
                lista_filtrada_por_nome.append(product)

        return jsonify(lista_filtrada_por_nome)


    return jsonify(products)

def filtrar_por_nome(products, nome):
    lista_filtrada_por_nome = []
    ...
    return lista_filtrada_por_nome

@app.route('/products/baratos', methods=['GET'])
def products_baratos():
    valor_maximo = 200
    lista_baratos = []

    for product in products:
        if product['preco'] < valor_maximo:
            lista_baratos.append(product)

    return jsonify(lista_baratos)
